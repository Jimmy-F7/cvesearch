...
import { createAlertRuleForUser, listAlertRulesForUser } from "@/lib/workspace-store";
...
export const GET = withRouteProtection(async function GET(request: NextRequest) {
  ...
  const response = NextResponse.json(await listAlertRulesForUser(session.userId));
...
export const POST = withRouteProtection(async function POST(request: NextRequest) {
  ...
  const response = NextResponse.json(await createAlertRuleForUser(session.userId, name, normalizeSearchState(body.search)));
  return applyWorkspaceSession(response, session);
}, {
  route: "/api/alerts",
